...
def CALE():
    ...
    convh2 = Conv1D(64, 3, padding='same', kernel_regularizer=l2(reg_strength))(convh1)
    convh2 = BatchNormalization()(convh2)
    convh2 = Activation('relu')(convh2)
    convh2 = MaxPooling1D(pool_size=1)(convh2)  
    convh3 = Conv1D(32, 2, padding='same', kernel_regularizer=l2(reg_strength))(convh2)
    convh3 = BatchNormalization()(convh3)
    convh3 = Activation('relu')(convh3)
    convh3 = MaxPooling1D(pool_size=1)(convh3)  
    convh4 = Conv1D(32, 2, padding='same', kernel_regularizer=l2(reg_strength))(convh3)
...
def CRLE():
    ...
    out = Dense(1)(dense)
    model = Model(inputs=convH, outputs=out)
    model.compile(optimizer=RMSprop(learning_rate=5e-5), loss='mse', metrics=['mae'])
    return model

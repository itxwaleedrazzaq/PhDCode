...
class Conv2DLayer:
    ...
    def forward(self, x):
        ...
        return self.layer.call(self.x).numpy()
    ...
    def backward(self, R, gamma=0.3):
        ...
        Z_pos = tf.nn.conv2d(self.x, weights_pos, strides=self.strides, padding=self.padding.upper(), dilations=self.dilation_rate)
        Z_neg = tf.nn.conv2d(self.x, weights_neg, strides=self.strides, padding=self.padding.upper(), dilations=self.dilation_rate)
        ...
        C = tf.nn.conv2d_transpose(S, self.weights + gamma * weights_pos, input_shape, strides=self.strides, padding=self.padding.upper(), dilations=self.dilation_rate)
